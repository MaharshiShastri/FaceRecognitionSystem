...
class LoginSystem:
        ...
        while True:
            if video_stream.stopped is True:
                ...
                face_names):
                    ...
                    # the WebCam
                    cv2.putText(frame, f"{face_names}", (left -20, bottom + 15), \
    ...
    def isPresent(self, UID):
        try:
            ...
            cur = conn.cursor()
            cur.execute("select * from employee_register where uid=%s", (UID,))
            row = cur.fetchone()
    ...
    def loginAdmin(self):
        ...
        if type(username) == None or type(password) == None:
            ...
            try:
                ...
                messagebox.showerror("Error!",f"Error due to {e}",parent=self.window)
    ...
    def registerPage(self):
        ...
        pic = Label(self.frame1, text = "Upload photo", font = (st.font2, 15, "bold"), bg = st.color1).place(x = 40, y = 389)
        upload = Button(self.frame1, text = "Upload", font = (st.font3, 12), bd = 2, command = self.select_file, cursor = "hand2", bg = st.color5, fg = st.color1).place(x = 40, y = 419)
        submitButton = Button(self.frame1, text='Submit', font=(st.font3, 12), bd=2, command=self.submitData, cursor="hand2", bg=st.color5,fg=st.color1).place(x=200,y=450,width=100)
    ...
    def getUID(self):
        try:
            conn = mysql.connector.connect(
                ...
                database=cr.database)
            cur = conn.cursor()
            cur.execute("select MAX(uid) from employee_register")
            row = cur.fetchone()
    ...
    def submitData(self):
        if self.nameEntry.get() == "" or self.surnameEntry.get() == "" or self.emailEntry.get() == "" or self.designationEntry.get() == "" or self.contactEntry.get() == "" or self.dobEntry.get() == "" or self.joinningDateEntry.get() == "" or self.genderEntry.get() == "" or self.addressEntry.get() == "":
            ...
            try:
                ...
                cur.execute("insert into employee_register (f_name,l_name,email,designation,contact,dob,join_date,gender,address) values(%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                                        (
                                            self.nameEntry.get(),
    ...
    def select_file(self):
        row = self.getUID()
        selected_directory = filedialog.askopenfilename(filetypes=[("JPG Files", "*.jpg"), ["PNG Files", "*.png"]])
        if selected_directory is not None:
            file_name  = f"\{row[0] + 1}.png"
            os.chdir("../")
            save_directory = str(os.getcwd()) + "\Face-Recognition-Login-System-main\Images" + file_name
            destination = Path(save_directory)
            shutil.move(selected_directory, destination)
# The main function
